...
<form method="post" action="<?php if (empty ( $this->_aVars['sCategoryUrl'] )):  echo Phpfox::getLib('phpfox.url')->makeUrl($this->_aVars['sParentLink']);  else:  echo Phpfox::getLib('phpfox.url')->makeUrl(''.$this->_aVars['sParentLink'].'.category'.$this->_aVars['sCategoryUrl'].'');  endif; ?>">
<?php echo '<div><input type="hidden" name="' . Phpfox::getTokenName() . '[security_token]" value="' . Phpfox::getService('log.session')->getToken() . '" /></div>'; ?>
<div>
<?php echo $this->_aVars['aFilters']['keyword']; ?>
<input type="submit" value="<?php echo Phpfox::getPhrase('core.submit'); ?>" id="search_button"  name="search[submit]" style="text-indent:-9999px;background:url('<?php echo Phpfox::getParam('core.url_module'); ?>recipe/static/image/searchbutton.png') no-repeat 0 0;"/>
</div>
...
<?php echo Phpfox::getPhrase('recipe.search_tips'); ?>
</div>
