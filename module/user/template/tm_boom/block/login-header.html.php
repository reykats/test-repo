{if Phpfox::getLib('module')->getFullControllerName() != 'user.login'}
									...
									{/if}
										<form method="post" action="{url link='user.login'}">
											...
											<div class="header_menu_login_button">
												...
												<div class="close"><a href="javascript:void(0)" onclick="{literal}$('#header_menu_login').animate({top:'-70px'},500);{/literal}">close</a></div>
											</div>
										</form>
									{if isset($bCustomLogin)}
									</div>									
								...
								<div id="login_links">
									<ul>
										<li class="signup_link"><a href="{url link='user.register'}">Signup</a></li>
										<li class="login_link"><a href="javascript:void(0)" onclick="{literal}$('#header_menu_login').animate({top:'0px'},500);{/literal}">Login</a></li>
									</ul>
								</div>
								{/if}